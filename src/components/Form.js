...
export default function Form(props) {
    ...
    let { form, setForm } = useContext(appContext);
    ...
    return (
        <div>
            <form className='border border-success rounded p-2'>
                ...
                <div className="mb-2">
                    <label htmlFor="date" className="form-label m-2">Patient ID</label>
                ...
                <div className="container d-flex p-2 justify-content-center">
                    <Link to="/Appointment"><button className="btn btn-success btn-md m-1" style={{ display: (props.parent === "refill") ? "none" : "block" }}>Book an Appointment</button></Link>
                    <Link to="/Refills"><button id="refill" className="btn btn-secondary btn-md m-1">Refill your Prescription</button>
                    </Link>
                </div>
            </form>
        </div>
